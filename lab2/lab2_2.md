...
```r
#Value comes up as TRUE at 15
#Can't do my_vector_sequence = 15 because it makes new vector. It overwrites and makes my_vector_sequence value only 15
```
...
```r
#my_vector_sequence < 10 only 9 trues. If you wanted 10 trues, you need to make 10 <=
```
...
```r
#Pulling everything out of column 2. non_us_earnings is the total of all the values, so type in mean(non_us_earnings) to find mean
