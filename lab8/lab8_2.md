...
(Do not put set or get working directory in your R code! You're basically saying, "This is only going to work on my computer!" Passive aggressive sassy, so don't do it!)
...
```r
expenditures %>% #Hold expenditure value
  pivot_longer(-expenditure, #Keeping expenditure column
               names_to = "year",
...
The data are not tidy. THis is because the column names themselves are variables, and in each column, there are different spending values

-Now you have X in year, Y in bn_dollars
...
Sex of characters inlcuded. Are the data tidy? No, because each cell has more than one observation, ex. Margaret female, Frank male. You deal with this with the separate command
...
```r
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = "_") #Separating patient (name of column) into patient and sex, and want it to be separate by underscore
```
...
Data are not tidy because you need to make a, b, c, d their own column
...
heartrate3 <- heartrate2 %>% 
  ...
  pivot_longer(-c(patient, sex), #Hold values patient and sex
               names_to = "drug",
               values_to = "heartrate")
```
...
In long format. Countries are a variable. Year, key, and then value. Issue: this is not convenient if presenting in a data table to a publication.
...
^New column names come from key, values come from value.
...
Have long data, but you need to make it wide. Where will the names come from? Type
...
This format is more legible, but less analyzable.
Why are these data not tidy? Whether a number is treatment or control is apart of the data. Treatment/control is an observation, it's part of the data
...
Combine sex and race of observations.
...
pivot_wider(names_from = (education_level), #new column names come from the education_level column
                values_from = c(mean_income, n)) #values come from two separate columns (Because two different education levels)
```
