...
-You end up with a blank plot, but things are already where they need to be! You have a ranged of petal length, etc. You're just missing the kind of plot you want to make, which is the geom type. 
...
```r
#You can switch X and Y axis by respecifying x and y values
```
...
```r
#Shows proportion of missing data
#Know how NAs are represented and where they are concentrated
```
...
-log10.mass is because there's a lot of animals in the data, and the range gets so different (ex. mouse vs. elephant) that log function is used


```r
ggplot(data = homerange, mapping = aes(x = log10.mass, y = log10.hra)) + geom_point() #Notice it's a +, not a %>% 
```

![](lab9_1_files/figure-html/unnamed-chunk-12-1.png)<!-- -->

-This shows the bigger you are, the larger your homerange. Positive trend/correlation!
...
-Somewhat minor difference in this example, but this can be useful for other data!
...
```r
#Geom smooth is adding another layer
#Smoothing is a smoothing function
#SE is standard regression error
```
...
-SHOULD be an alert where it says there are 502 rows that contain missing (geom_point). It shows up if na.rm=T wasn't inserted
...
-Only counts the number of times it sees a certain category in a categorical variable. You would not use this with a continuous variable
...
-A good practice is to always provide the table with the graph. Make the analysis of the data clear to the reader
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+ #Common name of members of salmonidae, then log mass on y axis
  geom_col() #Not geom_bar() in this case. The next example shows you how you can if you want to though!
```
...
-Important that you use geom_bar for counts, and geom_col for specifying an x and y axis
...
```
-If you ever get lost, you can use summary functions like names(homerange) or glimpse(homerange) to remember what is called what!
...
-If x and y are flipped, it just flips graph, which can be helpful depending on how you want things arranged
...
##    taxon common.name class order family genus species primarymethod N    
##    <chr> <chr>       <chr> <chr> <chr>  <chr> <chr>   <chr>         <chr>
##  1 mamm… Northern t… mamm… dide… didel… mono… americ… telemetry*    <NA> 
##  2 mamm… southern g… mamm… roden crice… onyc… torrid… telemetry*    <NA> 
##  3 mamm… salt marsh… mamm… rode… crice… reit… ravive… telemetry*    <NA> 
##  4 mamm… wood mouse  mamm… rode… murid… apod… sylvat… telemetry*    <NA> 
##  5 mamm… greater wh… mamm… sori… soric… croc… russula telemetry*    <NA> 
##  6 mamm… arctic shr… mamm… sori… soric… sorex arctic… telemetry*    <NA> 
##  7 mamm… cinereus s… mamm… sori… soric… sorex cinere… telemetry*    <NA> 
##  8 mamm… crowned sh… mamm… sori… soric… sorex corona… telemetry*    <NA> 
##  9 mamm… slender sh… mamm… sori… soric… sorex gracil… telemetry*    <NA> 
## 10 mamm… long-clawe… mamm… sori… soric… sorex unguic… telemetry*    <NA> 
...
homerange %>% 
  ...
  top_n(-10, log10.mass) %>%
  ggplot(aes(x=common.name, y=log10.mass))+
  geom_col() #Since you want to add 2 variables specified, needs to be geom_col
```

![](lab9_1_files/figure-html/unnamed-chunk-31-1.png)<!-- -->

-Some of this looks messy, so what can you do? Do below!
...
-Nice if you can see data in an orderly way, so do this by adding in x=reorder
-x=reorder, common name reordered by log10.mass
